...
<div class="breadcrumb-container ml-2 mt-2">
    <nav aria-label="breadcrumb">
        <ol class="breadcrumb bg-white">
            ...
            <li class="breadcrumb-item"><a href="{{ url('/shop/category/' . $category_id) }}">{{ $category_name }}</a>
            </li>
...
<div class="row ml-3 mb-5">
    ...
    <div class="col-9 col-sm-9 col-lg-4">
        ...
        <div class="row product-buttons mt-5">
            ...
            <div class="col-2">
                <a data-sku="{{ $product->sku }}" data-price="{{ $product->price }}" data-order-type="0"
                    class="btn btn-add-cart"><img
            ...
            @if ($product->rebranding == 1)
                <div class="col-12">
                    <a href="{{ url('/rebrand/' . $product->sku . '/' . $category_name) }}"
                        class="btn btn-outline-secondary btn-block m-1">Rebrand now!</a>
                </div>
            @endif
        </div>
    </div>
    <div class="col-md-12 col-lg-5">
        <div class="ml-3 mt-4 product-information">
                ...
                @if ($variation)
                    @foreach ($variation as $key => $item)
                        ...
                        @if (count($variation) > 0 && $item->variation != null)
                            <button class="btn btn-light btn-variation {{ $active }}"
                                data-sku="{{ $item->sku }}">{{ $item->variation }}</button>
                        @else
            ...
            <div class="p-details mt-2" id="detail-hide-direction" style="height:53px;">
                <div class="text-bold">Directions <span class="btn float-right btn-show-hide"
                        object="direction">+</span></div>
                <span id="dots-btn-direction">&#8203;</span><span
                    id="direction-text">{{ $product->directions }}</span>
            </div>
            <div class="p-details mt-2" id="detail-hide-precaution" style="height:53px;">
                <div class="text-bold">Precautions <span class="btn float-right btn-show-hide"
                        object="precaution">+</span></div>
                <span id="dots-btn-precaution">&#8203;</span><span
                    id="precaution-text">{{ $product->precautions }}</span>
            </div>
            <div class="p-details mt-2" id="detail-hide-ingredient" style="height:53px;">
                <div class="text-bold">Ingredients <span class="btn float-right btn-show-hide"
                        object="ingredient">+</span></div>
                <span id="dots-btn-ingredient">&#8203;</span><span
                    id="ingredient-text">{{ $product->ingredients }}</span>
            </div>
